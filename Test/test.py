...
def check_precision_recall(loader, model):
    num_correct = 0
    num_samples = 0
    tp = 0
    fp = 0
    ...
    fpr = fp/(tn+fp)
    model.train()
    return precision, recall, f1, fpr, pred, label
...
print(f"Accuracy on test set: {check_accuracy(dataloader, net)*100:2f}")
precision, recall, f1, fpr, pred, label = check_precision_recall(dataloader, net)
print(f"Precision, recall, f1, fpr: {precision, recall, f1, fpr}")
roc_auc, avg_precision = calculate_auc_scores(dataloader, net)
